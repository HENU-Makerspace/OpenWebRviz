            ...
            <div className="map-frame relative min-h-0 overflow-hidden" style={{ backgroundImage: 'url(' + borderImage + ')' }}>
              ...
              {rightViewport === 'camera' ? (
                ...
                <div className="main-camera-view relative z-10 flex h-full items-center justify-center overflow-hidden rounded-lg bg-slate-950/42 p-4">
                  <MediaViewport videoRef={media.videoRef} audioRef={media.audioRef} videoConnected={media.videoConnected} audioMonitoring={media.audioConnected} talkbackActive={media.talkbackActive} microphoneGain={media.microphoneGain} loadingAction={media.loadingAction} error={media.error} faceSnapshot={face.snapshot} onRefresh={() => void media.refreshStatus()} onToggleVideo={() => { if (media.videoConnected) { void media.stopVideo(); return; } void media.startVideo(); }} onToggleAudio={() => { if (media.audioConnected) { media.stopAudioMonitor(); return; } void media.startAudioMonitor(); }} onToggleTalkback={() => { if (media.talkbackActive) { void media.stopTalkback(); return; } void media.startTalkback(); }} onMicrophoneGainChange={media.setMicrophoneGain} />
                </div>
              )}
            </div>
